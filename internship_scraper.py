        ...
        for card in job_cards:
            try:
                ...
                if title_elem and company_elem:
                    ...
                    title_lower = title.lower()
                    is_spring_2026 = "2026" in title_lower and any(
                        term in title_lower
                        for term in ["spring", "january", "february", "jan", "feb"]
                    )

                    if (
                        "intern" in title_lower
                        and is_spring_2026
                        and self.is_tech_role(title)
                    ):
                        internships.append(
                            {
                                "company": company,
        ...
        for card in job_cards:
            try:
                ...
                if title_elem and company_elem:
                    ...
                    apply_link = (
                        "https://indeed.com" + apply_link_elem.get("href")
                    ...
                    if "just posted" in date_raw.lower() or "today" in date_raw.lower():
                        ...
                        "day ago" in date_raw.lower() or "days ago" in date_raw.lower()
                    ):
                        days = int("".join(filter(str.isdigit, date_raw)) or 1)
                        date_posted = (today - datetime.timedelta(days=days)).strftime(
                            "%Y-%m-%d"
                        )
                    else:
                        date_posted = today.strftime("%Y-%m-%d")
        ...
        for card in job_cards:
            try:
                ...
                if title_elem and company_elem:
                    ...
                    date_posted = datetime.datetime.now().strftime(
                        "%Y-%m-%d"
                    )  # Glassdoor doesn't always show posting date
                    apply_link = (
                        "https://www.glassdoor.com" + apply_link_elem.get("href")
                        if apply_link_elem and apply_link_elem.get("href")
                        else "#"
                    )
    ...
    def scrape_site(self, site):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"Failed to scrape {site['name']}: Status code {response.status_code}"
                )
                return []
        except Exception as e:
